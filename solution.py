    ...
    for i in range(self.number_limbs):
      for j in range(self.number_elements_per_limb[i]):
        ...
        if j == 0:
          ...
          pyrosim.Send_Joint(name = name_link_torso , parent= names_body_elements[self.location_on_main_body_limb[0]] , child = names_body_elements[self.location_on_main_body_limb[i]] , type = "revolute", position = [self.limb_joint_element_x[i][j],self.limb_joint_element_y[i][j],self.limb_joint_element_z[i][j]], jointAxis = "0 0 1")
