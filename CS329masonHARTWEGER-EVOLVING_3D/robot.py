...
class ROBOT:
  ...
  def Think(self):
    self.nn.Update()
    #self.nn.Print()
  def Get_Fitness(self,call_num,checkpoint_current_fitness):
    print(call_num)
    self.comp = 0
    if call_num >= 2:
      self.comp = checkpoint_current_fitness + 10
    stateOfLinkZero = p.getLinkState(self.robotId,0)
    positionOfLinkZero = stateOfLinkZero[0]
    xCoordinateOfLinkZero_difference = abs(positionOfLinkZero[0]) #- self.comp
    #if self.iteration == 3:
    #  self.xCoordinateOfLinkZero_difference_COMP = xCoordinateOfLinkZero_difference
    #if self.iteration > 3:
    #  xCoordinateOfLinkZero_difference = xCoordinateOfLinkZero_difference - self.xCoordinateOfLinkZero_difference_COMP - 10
    #basePositionAndOrientation = p.getBasePositionAndOrientation(self.robotId)
    #basePosition = basePositionAndOrientation[0]
    #xPosition = basePosition[0]
    print(str(xCoordinateOfLinkZero_difference))
